...
import { TOTAL_EXPENSES_TODAY } from "../../api-routes";
const Kpi = () => {

    const [todayTotalExpense,setTodayTotalExpense]=useState(0)
    const monthlySalary=Cookies.get("monthlySalary")
    const getTodayTotalExpenses=async()=>{
        try {
          const response=  await api.get(TOTAL_EXPENSES_TODAY);
          if(response.status===200){
            console.log(response.data)
            setTodayTotalExpense(response.data.data.totalAmountPerDay)
          }
            ...
            <div className='d-flex flex-wrap align-items-center justify-content-between gap-1 mb-8'>
              <div className='d-flex align-items-center gap-10'>
                ...
                <div>
                  ...
                  <h6 className='fw-semibold mt-2 '>{todayTotalExpense?`₹ ${ todayTotalExpense}` :0.00}</h6>
                </div>
              </div>
            </div>
            ...
            <div className='d-flex flex-wrap align-items-center justify-content-between gap-1 mb-8'>
              <div className='d-flex align-items-center gap-10'>
                ...
                <div>
                  ...
                  <h6 className='fw-semibold mt-2 '>&#8377; {monthlySalary}</h6>
                </div>
              </div>
            </div>
            {/* <p className='text-sm mb-0 d-flex align-items-center flex-wrap gap-12 mt-12 text-secondary-light'>
